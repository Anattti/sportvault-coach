-- Coach workout authorization helpers and secured RPCs
-- Applied in production as migration version 20260606140000

CREATE OR REPLACE FUNCTION public.can_manage_user_workouts(p_target_user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE coach_id = (SELECT auth.uid())
          AND client_id = p_target_user_id
          AND status = 'active'
      )
    );
$$;

CREATE OR REPLACE FUNCTION public.can_manage_workout(p_workout_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE w.id = p_workout_id
      AND public.can_manage_user_workouts(w.user_id)
  );
$$;

REVOKE ALL ON FUNCTION public.can_manage_user_workouts(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.can_manage_workout(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.can_manage_user_workouts(uuid) TO authenticated;
